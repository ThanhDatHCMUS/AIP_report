...
LIMIT 5;
            ...
            if not rows:
                return json.dumps({"message": "🔻 Top 5 cổ phiếu giảm mạnh nhất: Không có dữ liệu."}, ensure_ascii=False, indent=4)
            ...
            return json.dumps({"error": f"❌ Lỗi khi chạy truy vấn giảm mạnh nhất: {str(e)}"}, ensure_ascii=False, indent=4)
...
LIMIT 1
            ...
            if not row:
                return json.dumps({"message": f"Không có dữ liệu về khối lượng giao dịch cho ngày {date_key}."}, ensure_ascii=False, indent=4)
            ...
            return json.dumps({"error": f"❌ Lỗi khi truy vấn khối lượng giao dịch cao nhất: {str(e)}"}, ensure_ascii=False, indent=4)
...
LIMIT 1
            ...
            if not row:
                return json.dumps({"message": "Cổ phiếu có biên độ dao động mạnh nhất: Không có dữ liệu."}, ensure_ascii=False, indent=4)
            ...
            return json.dumps({"error": "Cổ phiếu có biên độ dao động mạnh nhất: Không có dữ liệu."}, ensure_ascii=False, indent=4)
# import asyncio
# import asyncpg
# from datetime import datetime

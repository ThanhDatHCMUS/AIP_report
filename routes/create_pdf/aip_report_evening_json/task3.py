        ...
        try:
            ...
            result_json = {
                "Nhóm ngành tăng mạnh nhất": {
                    ...
                    "increase_percent": round(df_high_5['increase_percent'].values[0] * 100, 2)
                },
                "Nhóm ngành giảm mạnh nhất": {
                    "name": df_bottom['icb_name2'].values[0],
                    "increase_percent": round(df_bottom['increase_percent'].values[0] * 100, 2)
                },
                "Hiệu suất một số nhóm ngành chính": {
                    "Ngân hàng": {
                        "status": "Tăng" if increase_bank >= 0 else "Giảm",
                        "percent": round(increase_bank * 100, 2) if increase_bank is not None else None
                    },
                    "Bất động sản": {
                        "status": "Tăng" if increase_bds >= 0 else "Giảm",
                        "percent": round(increase_bds * 100, 2) if increase_bds is not None else None
                    },
                    "Dầu khí": {
                        "status": "Tăng" if increase_dk >= 0 else "Giảm",
